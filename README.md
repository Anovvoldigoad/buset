...
| 19 | **[Fix runtime, root cause ketemu]** `crash.log` menunjukkan `System.InvalidOperationException: Cannot find non-neutral culture related to 'en-us'` di `XmlLanguage.GetSpecificCulture()` — dipicu saat WPF resolve `Language="en-US"` untuk sebuah `Binding` (`BitmapImage.UriSource`) pas startup window pertama. Penyebab: .NET butuh ICU untuk resolusi culture spesifik, dan ICU di Wine/Winlator sering tidak lengkap. Fix: `<InvariantGlobalization>true</InvariantGlobalization>` (permanen, di-bake ke `runtimeconfig.json`) + `DOTNET_SYSTEM_GLOBALIZATION_INVARIANT=1` di launcher (jaring pengaman). Lokalisasi kustom app (`lang.xaml`) tidak terpengaruh — itu resource XAML biasa, bukan `CultureInfo` .NET | `NSC-ModManager.csproj`, `Launch-Winlator.bat` |

### Status

Ini kandidat fix yang **jelas dan langsung ke akar masalah** (bukan tebakan seperti sebelumnya) — sudah ada bukti pasti dari `crash.log` yang menunjuk tepat ke satu baris kode WPF internal yang gagal karena ICU. Tetap disarankan uji langsung: kalau setelah ini masih ada crash lain, `crash.log` akan tetap ditulis (handler-nya permanen, bukan sekali pakai) jadi tinggal kirim isinya lagi kalau ada masalah baru.
