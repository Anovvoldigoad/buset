...
| 23 | **[BUG DARI ASISTEN, bukan dari repo]** `crash.log` kelima menunjukkan crash BARU: `MS.Internal.FontCache.MajorLanguages` (subsistem font/text-rendering WPF) throw `CultureNotFoundException: Only the invariant culture is supported in globalization-invariant mode`. Ternyata **`DOTNET_SYSTEM_GLOBALIZATION_INVARIANT=1` di `Launch-Winlator.bat` tidak pernah dihapus** waktu csproj-nya direvert di fix #19a — env var ini SENDIRIAN cukup memaksa Invariant Globalization Mode aktif, independen dari setting csproj. Akibatnya, **semua percobaan sejak fix #19a sebenarnya masih jalan di mode invariant tanpa disadari**, yang kemungkinan besar juga ikut menutupi/mengganggu efektivitas fix #21 (`Language=""`) dan #22 (`ConverterCulture=''`) di percobaan-percobaan sebelumnya. WPF **tidak bisa jalan sama sekali** di Invariant Globalization Mode — `MajorLanguages` hardcode `new CultureInfo("en")` di static constructor, pasti crash begitu ada `TextBlock` apa pun yang dirender. Env var dihapus total dari launcher, dengan komentar tebal "JANGAN AKTIFKAN LAGI" supaya tidak terulang | `Launch-Winlator.bat` |

### Catatan jujur soal fix #23

Ini murni kesalahan proses debugging saya sendiri (lupa membersihkan env var setelah revert csproj), bukan temuan baru dari sistem. Ada kemungkinan nyata bahwa fix #21 dan #22 sebenarnya **sudah benar sejak awal** dan cuma ketutup oleh bug invariant-mode yang gak sengaja ini. Setelah env var ini dihapus, kemungkinan besar app akan lolos lebih jauh lagi — tapi kalau crash "cannot find non-neutral culture" yang LAMA muncul lagi di titik yang BELUM ditambal (ada `TextBlock`/`Binding` lain yang belum ketahuan), itu bukan berarti fix #21/#22 salah, cuma berarti ada titik lain yang perlu ditambal dengan pola yang sama (`ConverterCulture=''` di Binding terkait, atau `Language=""` kalau di root Window baru).
